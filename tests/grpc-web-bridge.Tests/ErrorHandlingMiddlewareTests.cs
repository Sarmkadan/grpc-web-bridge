...
public sealed class ErrorHandlingMiddlewareTests
{
	...
	/// <summary>
	/// Tests that the ErrorHandlingMiddleware class returns a 400 status code when a ConfigurationException is thrown.
	/// </summary>
	[Fact]
	public async Task InvokeAsync_WithConfigurationException_Returns400()
	{
		var (status, body) = await ExecuteAsync(new ConfigurationException("Config key error"));

		status.Should().Be(StatusCodes.Status400BadRequest);
		body.Should().Contain("Configuration Error");
	}

	/// <summary>
	/// Tests that the ErrorHandlingMiddleware class returns a 400 status code when a ConfigurationException with key and value is thrown.
	/// </summary>
	[Fact]
	public async Task InvokeAsync_WithConfigurationExceptionWithKey_Returns400()
	{
		var (status, body) = await ExecuteAsync(new ConfigurationException("testKey", "testValue", "Invalid configuration"));
	...
	/// <summary>
	/// Tests that the ErrorHandlingMiddleware class returns a 400 status code when a ValidationException is thrown.
	/// </summary>
	[Fact]
	public async Task InvokeAsync_WithValidationException_Returns400()
	{
		var (status, body) = await ExecuteAsync(new ValidationException("Field is required"));
	...
	/// <summary>
	/// Tests that the ErrorHandlingMiddleware class returns a 400 status code when a ValidationException with field details is thrown.
	/// </summary>
	[Fact]
	public async Task InvokeAsync_WithValidationExceptionWithField_Returns400()
	{
		var (status, body) = await ExecuteAsync(new ValidationException("email", "invalid-email", "email", "Email format is invalid"));
